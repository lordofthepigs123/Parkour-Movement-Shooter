...
public class PlayerColliderManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //ground raycast
        //Debug.DrawRay(RotAdjustPosition(), Vector3.down * groundCheckDis, Color.blue,1);
        if (Physics.Raycast(RotAdjustPosition(), Vector3.down, groundCheckDis, whatIsGround) || touchingGround)
            grounded = true;
        else
            grounded = false;
    }
    ...
    public Vector3 RotAdjustPosition()// Position that shifts towards ground and center when tilted
    {
        ...
        return transform.position + 0.5f * 0.99f * tempHeight * Vector3.down + ActiveHeight / 2.01f * tempHeight * tempFwd + Vector3.up * insideCheckDis;
    }

    public float SlopeAngle()
    {
        return Vector3.Angle(Vector3.up, wallNormal);
    }
}
